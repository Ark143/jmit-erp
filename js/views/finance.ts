...
import { formatMoney, getPrintHeaderHtml, getPrintFooterHtml, renderAuditTrailSection, renderJEPreview } from "../utils";
...
function renderPaymentDetails(container, paymentId) {
  ...
  if (isDraft) {
    ...
    const postedJe = store.getJournalEntries().find((je: any) => je.reference.includes(pay.id));
