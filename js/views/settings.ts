...
function renderUsers(container) {
            ...
            <tbody>
              ${roles.map((r, ri) => {
                const activeModules = Object.entries(r.permissions)
                  .filter(([_, p]: [string, any]) => p.read || p.create || p.update || p.delete || p.approve)
  ...
  container.querySelectorAll(".save-perms-inline-btn").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      const newPerms: any = { o2c:{}, p2p:{}, inventory:{}, accounting:{}, finance:{}, settings:{} };
      const actions = ["read","create","update","delete","approve"];
      ["o2c","p2p","inventory","accounting","finance","settings"].forEach(mod => {
        actions.forEach(a => { newPerms[mod][a] = false; });
      });
      container.querySelectorAll(`.perm-cb-inline[data-roleid="${roleId}"]`).forEach((cb: HTMLInputElement) => {
        if (cb.checked) newPerms[cb.dataset.mod][cb.dataset.action] = true;
      });
      try {
        store.updateRole(roleId, { permissions: newPerms });
        const msg = container.querySelector(`.perms-saved-msg-${roleId}`) as HTMLElement;
...
function openRoleModal(roleId, onClose) {
  ...
  modal.querySelector("#role-form").addEventListener("submit", (ev) => {
    ev.preventDefault();
    const name = (modal.querySelector("#role-name") as HTMLInputElement).value.trim();
    if (!name) { window.showToast("Role name required", "warning"); return; }

    const newPerms: any = {};
    modulesList.forEach(m => { newPerms[m.key] = { create: false, read: false, update: false, delete: false, approve: false }; });
    modal.querySelectorAll("input[type=checkbox]").forEach((cb: HTMLInputElement) => {
      if (cb.checked) {
        const mod = cb.dataset.mod;
        const action = cb.dataset.action;
        if (newPerms[mod]) newPerms[mod][action] = true;
      }
    });
