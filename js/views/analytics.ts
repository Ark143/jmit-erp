...
function agingReport(title: string, items: any[], dateField: string, amountField: string, nameField: string, idField: string, statusFilter?: string[]): string {
  const aging: Record<string, { count: number; total: number; rows: string }> = {
  ...
  items.forEach((item: any) => {
    if (statusFilter && !statusFilter.includes(item.status)) return;
...
// ─── 1. AR AGING ───
export function renderARAging(container: HTMLElement, companyId: string) {
  const invoices = store.getSalesInvoices().filter(si => !companyId || si.companyId === companyId);
  container.innerHTML = agingReport(
    "Accounts Receivable (AR) Aging", invoices,
    "date", "total", "customerName", "id",
    ["Unpaid", "Draft", "Overdue"]
  );
}

// ─── 2. AP AGING ───
export function renderAPAging(container: HTMLElement, companyId: string) {
  const invoices = store.getPurchaseInvoices().filter(pi => !companyId || pi.companyId === companyId);
  container.innerHTML = agingReport(
    "Accounts Payable (AP) Aging", invoices,
    "date", "total", "vendorName", "id",
    ["Unpaid", "Draft"]
  );
}

// ─── 3. PO AGING (unfulfilled purchase orders) ───
export function renderPOAging(container: HTMLElement, companyId: string) {
  const pos = store.getPurchaseOrders().filter(po => !companyId || po.companyId === companyId);
  container.innerHTML = agingReport(
    "Purchase Order Aging (Unfulfilled)", pos,
    "date", "total", "vendorName", "id",
    ["Draft", "Approved"]
  );
}
...
// ─── 4. DELIVERY / SO AGING ───
export function renderSOAging(container: HTMLElement, companyId: string) {
  const sos = store.getSalesOrders().filter(so => !companyId || so.companyId === companyId);
  container.innerHTML = agingReport(
    "Sales Order Aging (Unfulfilled)", sos,
    "date", "total", "customerName", "id",
    ["Draft", "Approved"]
  );
}
...
// ─── 5. PAYMENT AGING ───
export function renderPaymentAging(container: HTMLElement, companyId: string) {
  const payments = store.getPayments().filter(p => !companyId || p.companyId === companyId);
  // Sort by date desc
  const sorted = [...payments].sort((a: any, b: any) => b.date.localeCompare(a.date));
  let rows = "";
...
export function renderInventoryAnalytics(container: HTMLElement, companyId: string) {
  const items = store.getItems();
  const wh = store.getWarehouses();
  let rows = "";
  let totalValue = 0;
  items.forEach((item: any) => {
    const totalQty = (item.stocks ? Object.values(item.stocks) as number[] : []).reduce((s: number, q: any) => s + Number(q), 0);
    const value = totalQty * (item.cost || 0);
    totalValue += value;
    const whBreakdown = wh.map((w: any) => `${w.name}: ${item.stocks?.[w.id] || 0}`).join(", ");
    rows += `<tr>
...
// ─── 7. PENDING APPROVALS ───
export function renderPendingApprovals(container: HTMLElement, companyId: string) {
  const soPending = store.getSalesOrders().filter((s: any) => s.status === "Draft" && (!companyId || s.companyId === companyId));
  const poPending = store.getPurchaseOrders().filter((p: any) => p.status === "Draft" && (!companyId || p.companyId === companyId));
  const dnPending = store.getDeliveries().filter((d: any) => d.status === "Draft" && (!companyId || d.companyId === companyId));
  const grnPending = store.getGoodsReceipts().filter((g: any) => g.status === "Draft" && (!companyId || g.companyId === companyId));
  const piPending = store.getPurchaseInvoices().filter((p: any) => p.status === "Draft" && (!companyId || p.companyId === companyId));
  const siPending = store.getSalesInvoices().filter((s: any) => s.status === "Draft" && (!companyId || s.companyId === companyId));

  function pendingCard(section: string, items: any[], routePrefix: string, idField: string, nameField: string) {
    let rows = "";
    items.forEach((x: any) => {
...
export function renderFixedAssetAnalytics(container: HTMLElement, companyId: string) {
  const assets = store.getFixedAssets().filter((a: any) => !companyId || a.companyId === companyId);
  let rows = "";
  let totalNBV = 0;
  let totalCost = 0;
  let totalDepr = 0;
...
export function renderStatementOfAccount(container: HTMLElement, companyId: string) {
  ...
  // Filter by company
  const filtInv = invoices.filter((i: any) => !companyId || i.companyId === companyId);
  const filtPay = payments.filter((p: any) => !companyId || p.companyId === companyId);

  let rows = "";
  partners.forEach((p: any) => {
    const custInv = filtInv.filter((i: any) => i.customerId === p.id || i.vendorId === p.id);
    const custPay = filtPay.filter((pm: any) => pm.partnerId === p.id);
    const totalBilled = custInv.reduce((s: number, i: any) => s + (i.total || 0), 0);
    const totalPaid = custPay.reduce((s: number, pm: any) => s + (pm.amount || 0), 0);
    const balance = totalBilled - totalPaid;
    if (totalBilled === 0 && totalPaid === 0) return;
    rows += `<tr>
...
export function renderAgingDashboard(container: HTMLElement, companyId: string) {
  const arInv = store.getSalesInvoices().filter((s: any) => !companyId || s.companyId === companyId).filter((s: any) => s.status !== "Paid");
  const apInv = store.getPurchaseInvoices().filter((p: any) => !companyId || p.companyId === companyId).filter((p: any) => p.status !== "Paid");
  const poOpen = store.getPurchaseOrders().filter((p: any) => !companyId || p.companyId === companyId).filter((p: any) => ["Draft", "Approved"].includes(p.status));
  const soOpen = store.getSalesOrders().filter((s: any) => !companyId || s.companyId === companyId).filter((s: any) => ["Draft", "Approved"].includes(s.status));

  function bucketTotal(items: any[], days: number) {
    return items.filter((i: any) => daysAgo(i.date) > days).reduce((s: number, i: any) => s + (i.total || 0), 0);
  }
  ...
  container.innerHTML = `<div class="animate-fade-in">
    ...
    <div class="grid-2" style="margin-bottom:16px;">
      <div class="card">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;">
          ...
          <span class="badge badge-draft">31-60: ${formatMoney(bucketTotal(arInv.filter((i: any) => daysAgo(i.date) <= 60 && daysAgo(i.date) > 30), -1))}</span>
          <span class="badge badge-pending">61-90: ${formatMoney(bucketTotal(arInv.filter((i: any) => daysAgo(i.date) <= 90 && daysAgo(i.date) > 60), -1))}</span>
      ...
      <div class="card">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;">
          ...
          <span class="badge badge-draft">31-60: ${formatMoney(bucketTotal(apInv.filter((i: any) => daysAgo(i.date) <= 60 && daysAgo(i.date) > 30), -1))}</span>
          <span class="badge badge-pending">61-90: ${formatMoney(bucketTotal(apInv.filter((i: any) => daysAgo(i.date) <= 90 && daysAgo(i.date) > 60), -1))}</span>
    ...
    <div class="grid-2">
      <div class="card">
        ...
        <p style="font-size:0.8rem;">${soOpen.length} open — ${formatMoney(soOpen.reduce((s: number, o: any) => s + (o.total || 0), 0))}</p>
      </div>
      <div class="card">
        <h4 style="color:var(--color-p2p);">PO Aging (Unfulfilled)</h4>
        <p style="font-size:0.8rem;">${poOpen.length} open — ${formatMoney(poOpen.reduce((s: number, o: any) => s + (o.total || 0), 0))}</p>
      </div>
    </div>
  </div>`;
}
