...
import { getPrintHeaderHtml, getPrintFooterHtml } from "../utils";

export function renderInventoryLedger(container: HTMLElement) {
  ...
  function refresh() {
    ...
    if (filterItem) filtered = filtered.filter((m: any) => m.itemId === filterItem);
    if (filterWarehouse) filtered = filtered.filter((m: any) => m.warehouseId === filterWarehouse);
    if (filterDoc) filtered = filtered.filter((m: any) => m.document === filterDoc);
    if (dateFrom) filtered = filtered.filter((m: any) => m.date >= dateFrom);
    if (dateTo) filtered = filtered.filter((m: any) => m.date <= dateTo);
    filtered.sort((a: any, b: any) => b.id.localeCompare(a.id) || b.date.localeCompare(a.date));

    const itemLookup: Record<string, any> = {};
    items.forEach(i => { itemLookup[i.id] = i; });
    const whLookup: Record<string, any> = {};
  ...
  const content = container.querySelector("#ledger-content") as HTMLElement;
  refresh();

  container.querySelector("#ledger-item")!.addEventListener("change", () => {
    filterItem = (container.querySelector("#ledger-item") as HTMLSelectElement).value;
    refresh();
  });
  container.querySelector("#ledger-wh")!.addEventListener("change", () => {
    filterWarehouse = (container.querySelector("#ledger-wh") as HTMLSelectElement).value;
    refresh();
  });
  container.querySelector("#ledger-doc")!.addEventListener("change", () => {
    filterDoc = (container.querySelector("#ledger-doc") as HTMLSelectElement).value;
    refresh();
  });
  container.querySelector("#ledger-from")!.addEventListener("change", () => {
    dateFrom = (container.querySelector("#ledger-from") as HTMLInputElement).value;
    refresh();
  });
  container.querySelector("#ledger-to")!.addEventListener("change", () => {
    dateTo = (container.querySelector("#ledger-to") as HTMLInputElement).value;
    refresh();
  });
  ...
  container.querySelector("#ledger-report-btn")!.addEventListener("click", () => {
    ...
    filtered.forEach((m: any) => {
      ...
      const wh = whLookup[m.warehouseId];
      const sign = m.type === "IN" ? "+" : "\u2212";
      if (m.type === "IN") totalIn += m.qty; else totalOut += m.qty;
      rows += `<tr>
        <td style="font-family:monospace;font-size:0.78rem;">${m.date}</td>
